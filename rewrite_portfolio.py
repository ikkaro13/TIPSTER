...
with open('backend/portfolio_manager.py', 'r', encoding='utf-8') as f:
    code = f.read()
    
idx1 = code.find('def place_bet')
part1 = code[:idx1]

new_funcs = """def place_bet(match_id, pick, odds, stake, evidence_snapshot=None, bet_type="PRE"):
...
final_code = final_code.replace('conn.commit()', f'conn.execute("""{audit_table_sql}""")\\n    conn.commit()', 1)

with open('backend/portfolio_manager.py', 'w', encoding='utf-8') as f:
    f.write(final_code)
