...
try:
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT league_id, team_id FROM elo')
    league_team_map = {}
    ...
    for name, lid in ALL_TRACKED_LEAGUES.items():
        if lid in league_team_map:
            ...
            if seeded_count < (total * 0.5): # Si menos de la mitad, lo consideramos faltante
                missing_leagues.append(name)
                total_tokens_needed += total
    
    print(f"Ligas faltantes: {missing_leagues}")
    print(f"Total de tokens necesarios para inyectarlas todas: {total_tokens_needed}")
